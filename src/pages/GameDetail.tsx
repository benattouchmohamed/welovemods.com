...
const GameDetail = memo(() => {
  ...
  /* ────────────────────── BEAUTIFUL SKY SKELETON ────────────────────── */
  const BeautifulSkeleton = () => (
    <div className="min-h-screen bg-sky-50/50 pt-24 px-4">
      <div className="max-w-5xl mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-8 items-start">
          <div className="md:col-span-5 lg:col-span-4 space-y-4">
            <div className="bg-white rounded-[2.5rem] p-4 shadow-sm">
              <div className="aspect-square bg-sky-100/50 rounded-[2rem] shimmer mb-4" />
              <div className="h-16 bg-sky-100/50 rounded-[1.8rem] shimmer" />
            </div>
          </div>
          <div className="md:col-span-7 lg:col-span-8 space-y-6">
            <div className="h-12 bg-sky-100/30 rounded-2xl w-3/4 shimmer" />
            <div className="h-6 bg-sky-100/20 rounded-full w-1/2 shimmer" />
            <div className="h-32 bg-white rounded-[2rem] shimmer border border-sky-50" />
          </div>
        </div>
      </div>
    </div>
  ...
  return (
    <>
      ...
      <main className="min-h-screen bg-[#F8FAFC] pt-6 pb-12 px-4">
        {!game ? (
          <div className="flex flex-col items-center justify-start pt-20 h-[80vh] text-center">
            <motion.div initial={{ scale: 0.5 }} animate={{ scale: 1 }}>
              <AlertCircle className="w-16 h-16 text-sky-400 mb-4 mx-auto" />
            </motion.div>
            <h1 className="text-2xl font-black text-slate-800">Mod not found</h1>
            {/* Moved button more UP */}
            <button 
              onClick={() => navigate("/")} 
              className="mt-4 px-10 py-3 bg-sky-500 text-white rounded-full font-black shadow-lg shadow-sky-200 active:scale-95 transition-all"
            >
              GO BACK HOME
            </button>
          </div>
          ...
          <div className="max-w-5xl mx-auto">
            <button onClick={() => navigate(-1)} className="mb-6 flex items-center gap-2 font-black text-slate-400 hover:text-sky-500 transition-colors uppercase tracking-[0.2em] text-[10px]">
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-8 items-start">
              ...
              <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="md:col-span-5 lg:col-span-4 space-y-4">
                <div className="bg-white rounded-[2.5rem] p-4 shadow-xl shadow-sky-900/5 border border-white">
                ...
                <div className="bg-white rounded-3xl p-5 border border-slate-100 flex justify-around items-center shadow-sm">
              ...
              <motion.div initial={{ x: 20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} className="md:col-span-7 lg:col-span-8 space-y-6">
                <div className="space-y-4 text-center md:text-left">
                  ...
                  <div className="flex flex-wrap justify-center md:justify-start gap-2">
                    <span className="bg-sky-50 text-sky-600 px-4 py-1.5 rounded-full font-black text-[10px] border border-sky-100 uppercase tracking-wider">Clean APK</span>
                ...
                <p className="text-lg text-slate-500 font-medium leading-relaxed italic border-l-4 border-sky-100 pl-6">
                  "{game.description}"
                </p>

                {/* Conditional Rendering for Mod Menu Details */}
                {game.features && game.features.length > 0 && (
                  <div className="bg-white rounded-[2.5rem] p-8 shadow-xl shadow-sky-900/5 border-t-8 border-sky-500">
                    <h3 className="font-black text-sky-500 text-[11px] tracking-[0.3em] uppercase mb-6 flex items-center gap-2">
                      <Zap size={14} className="fill-sky-500" /> Mod Menu Details
                    </h3>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      {game.features.map((f, i) => (
                        <div key={i} className="bg-slate-50 text-slate-700 px-5 py-4 rounded-2xl font-bold text-sm flex items-center gap-3 border border-slate-100">
                          <div className="w-2 h-2 rounded-full bg-sky-400" /> {f}
                        </div>
                      ))}
                    </div>
                  </div>
      ...
      <style>{`
        ...
        .shine { animation: shineMove 2.5s infinite; }
        @keyframes shineMove { 0% { transform: translateX(-100%) skewX(-12deg); } 100% { transform: translateX(500%) skewX(-12deg); } }
      `}</style>
    </>
  );
});
