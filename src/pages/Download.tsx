...
/* ────────────────────── UNIFIED OFFER CARD (no special "top" styling) ────────────────────── */
const OfferCard = memo(({ offer, onOpenModal }: { offer: Offer; onOpenModal: (o: Offer) => void }) => {
  const [locale] = useLocale();
  const i18n = t(locale);

  const icons: Record<string, JSX.Element> = {
    Smartphone: <Smartphone className="w-6 h-6 text-yellow-100" />,
    Monitor: <Monitor className="w-6 h-6 text-yellow-100" />,
    Gamepad2: <Gamepad2 className="w-6 h-6 text-yellow-100" />,
    Gift: <Gift className="w-6 h-6 text-yellow-100" />,
  };

  return (
    <article className="bg-white rounded-xl p-4 border-4 border-orange-400 shadow-xl overflow-hidden">
      <div className="flex gap-4">
        {/* Icon/Image */}
        <div className="w-16 h-16 rounded-lg bg-gradient-to-br from-pink-400 to-purple-500 p-2 flex-shrink-0 shadow-md flex items-center justify-center">
          {offer.image ? (
            <img src={offer.image} alt={offer.title} className="w-full h-full object-cover rounded-md" loading="lazy" />
          ) : (
            icons[offer.icon] || <DollarSign className="w-8 h-8 text-yellow-100" />
          )}
        </div>

        {/* Content */}
        <div className="flex-1 min-w-0">
          <h3 className="font-black text-base text-blue-600 line-clamp-2 mb-1">{offer.title}</h3>
          <p className="text-xs text-gray-600 line-clamp-2 mb-3">{offer.description}</p>
          ...
          <button
            ...
            className="w-full py-3 rounded-xl text-sm font-bold text-white bg-gradient-to-r from-emerald-500 to-teal-600 shadow-lg hover:shadow-xl hover:brightness-110 active:scale-95 transition-all duration-200"
          >
              ...
              <>
                ...
                <div className="space-y-5">
                  {offers.slice(0, 4).map((offer) => (
                    <OfferCard key={offer.id} offer={offer} onOpenModal={openModal} />
                  ))}
                </div>
              </>
            )}
