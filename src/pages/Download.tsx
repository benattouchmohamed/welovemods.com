...
import React, { useEffect, useState, useMemo, lazy, Suspense } from "react";
import { X, ShieldCheck, ChevronRight, Globe, Fingerprint, PlayCircle, Loader2, Lock, CheckCircle2, Users } from "lucide-react";
import { fetchOffers, type Offer } from "@/services/offerService";
import { useLocale, t } from "@/hooks/useLocale";
...
export default function DownloadPage() {
  ...
  useEffect(() => {
    fetch('https://ipapi.co/json/').then(r => r.json()).then(d => setUserCity(d.city || "Nearby")).catch(()=>{});
    const params = new URLSearchParams(window.location.search);
    setGameName(params.get("game") || "Premium Mod");
    setGameImage(sessionStorage.getItem("downloadGameImage"));
    ...
    const interval = setInterval(()=> setLiveUsers(u=>u+Math.floor(Math.random()*3)),3000);
  ...
  return (
    <div className="min-h-screen bg-[#FFFBEB] pb-10" dir={locale==='ar'?'rtl':'ltr'}>
      ...
      <div className="max-w-md mx-auto px-5 pt-4 flex justify-between items-center">
        <div className="flex items-center gap-2 bg-white px-3 py-1 rounded-full border">
          <Globe size={14}/> <span className="text-xs font-black">{userCity}</span>
        </div>
        <Suspense fallback={<div className="w-8 h-8 bg-gray-200 rounded-full"/>}><LangPicker/></Suspense>
      </div>
      ...
      <div className="max-w-md mx-auto px-5 mt-6 space-y-3">
        {offers.map((o,i)=>(
          <button key={o.id} onClick={()=>setSelectedOffer(o)}
            className={`w-full flex items-center gap-3 p-4 rounded-2xl border shadow transition ${
              completed.includes(i) ? "bg-green-50 border-green-400" : "bg-white"
            }`}>
            <div className="w-12 h-12 flex items-center justify-center rounded-xl bg-gray-100">
              {completed.includes(i)? <CheckCircle2 className="text-green-500"/>:<Lock/>}
            </div>
            <div className="flex-1 text-left">
              <h3 className="font-black">Task {i+1}</h3>
              <p className="text-xs text-gray-500">How to Verify</p>
            </div>
            <ChevronRight/>
          </button>
        ))}
      ...
      {selectedOffer && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-6 z-50">
          <div className="bg-white rounded-3xl p-6 max-w-sm w-full text-center relative">
            <button onClick={()=>setSelectedOffer(null)} className="absolute -top-3 -right-3 bg-red-500 text-white w-10 h-10 rounded-full flex items-center justify-center"><X/></button>
            <img src={selectedOffer.image||""} className="w-20 h-20 mx-auto rounded-2xl mb-3"/>
            <h3 className="font-black text-lg">How to Verify</h3>
            <p className="text-gray-600 text-sm mb-5">{selectedOffer.description}</p>

            <button onClick={()=>{
              window.open(selectedOffer.url,"_blank");
              setCompleted(p=>[...p, offers.indexOf(selectedOffer)]);
              setSelectedOffer(null);
            }}
            className="w-full bg-blue-600 text-white py-4 rounded-xl font-black">
              <Fingerprint className="inline mr-2"/> Verify Now
            </button>
          </div>
        </div>
      )}
