...
export const fetchGames = async (): Promise<Game[]> => {
  return [...games].sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());
};
...
export const fetchTopGames = async (): Promise<Game[]> => {
  return [...games].sort((a, b) => b.downloads - a.downloads).slice(0, 20);
};

export const fetchNewGames = async (): Promise<Game[]> => {
  return games.filter(g => g.is_new);
};

export const fetchHotGames = async (): Promise<Game[]> => {
  return games.filter(g => g.is_hot);
};

export const fetchGameBySlug = async (slug: string): Promise<Game | null> => {
  const byId = games.find(g => g.id === slug);
  if (byId) return byId;

  const createSlug = (title: string) =>
    title.toLowerCase().replace(/\s+/g, "-").replace(/[^\w-]/g, "");

  return games.find(g => createSlug(g.title) === slug) || null;
};
